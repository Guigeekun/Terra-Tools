...
├── local-input/                # User-supplied raw game assets and dumps (gitignored)
│   ├── terra-battle-5.5.7-170.apk # Terra Battle v5.5.7 APK
│   ├── dump.cs                 # C# structure dump generated by Il2CppDumper
...
├── dump.cs                        # [Required for Ch 8-42] C# dump from Il2CppDumper
...
2. **`dump.cs`** *(Required for Native Stage Layouts — Chapters 8 to 42)*
   - **Path**: `local-input/dump.cs` (configurable in `config.json`)
   - **Source**: Generated by running [Il2CppDumper](https://github.com/Perfare/Il2CppDumper) on `libil2cpp.so` and `global-metadata.dat` extracted from the APK.
   - **Used By**: `scripts/extract_native_stages.py` (and step 7 of `scripts/extract_everything.py`).
